...
subplot(2,1,1)
clm = params.mu + max(params.sd).*[-1 1];
t = (0:params.chordDuration:(length(DB{1,4,1,1})*params.chordDuration)) ...
    - 3;
imagesc(t,1:length(params.freqs),DB{1,4,1,1},clm); colorbar;
octs = find(mod(params.freqs,1000)==0);
set(gca,'ydir','normal'); % flip it to match frequency order
set(gca,'ytick',octs);
set(gca,'yticklabels',num2str(params.freqs(octs)'/1000));
...
saveFigPDF(f1,[500 250],sprintf('./hilo_noiseOnly_long.pdf',i));
